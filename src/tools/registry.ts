...
async function executeGetTcrRequest(
  ...
  try {
    ...
    if (!request) {
      return failure(name, 404, "TCR request not found.");
    }
    return success(name, request, SHORT_PRIVATE_CACHE_CONTROL);
  } catch (error) {
    return failure(name, 502, `get-tcr-request request failed: ${formatErrorMessage(error)}`);
  }
}
...
async function executeGetDispute(
  ...
  try {
    ...
    if (!dispute) {
      return failure(name, 404, "Dispute not found.");
    }
    return success(name, dispute, SHORT_PRIVATE_CACHE_CONTROL);
  } catch (error) {
    return failure(name, 502, `get-dispute request failed: ${formatErrorMessage(error)}`);
  }
}
...
async function executeGetStakePosition(
  ...
  try {
    ...
    if (!position) {
      return failure(name, 404, "Stake position not found.");
    }
    return success(name, position, SHORT_PRIVATE_CACHE_CONTROL);
  } catch (error) {
    return failure(name, 502, `get-stake-position request failed: ${formatErrorMessage(error)}`);
  }
}
...
async function executeGetPremiumEscrow(
  ...
  try {
    ...
    if (!escrow) {
      return failure(name, 404, "Premium escrow not found.");
    }
    return success(name, escrow, SHORT_PRIVATE_CACHE_CONTROL);
  } catch (error) {
    return failure(name, 502, `get-premium-escrow request failed: ${formatErrorMessage(error)}`);
  }
}
